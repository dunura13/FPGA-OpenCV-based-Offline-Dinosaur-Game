# =========================
# Online Dinosaur Game (Jump/Duck/Idle) -> UART (SH-U09C)
# Updated for FTDI FT232RL stability
# =========================

# --- Imports (os must come before os.environ use) ---
import os
...
# =========================
EMA_ALPHA = 0.6              # smoothing for yhat
VEL_DT_MIN = 1/60            # clamp dt for velocity (debug only)

# Position thresholds (normalized; + up, - down)
JUMP_Y_THRESH = +0.40        # consider +0.25 if too strict
DUCK_Y_THRESH = -0.40        # consider -0.25 if too strict
...
# =========================
SER_BAUD = 115200
ser = None  # global handle

def open_uart(port_hint: str = None):
    """
    Try to open the SH-U09C / FTDI FT232RL serial port.
    - If port_hint provided, use it
    - Else, auto-discover a likely FTDI/USB-Serial device.
    

    - Prevents DTR/RTS reset
    - Adds stabilization delay
    - Sends initial byte to prime connection
    ...
    if not port:
        # As a last resort on macOS, prefer /dev/cu.* for outbound connections
        for p in serial.tools.list_ports.comports():
    ...
    if not port:
        print("⚠️  No serial device found. Plug in the SH-U09C or pass a port_hint to open_uart().")
        return None
    ...
    try:
        ser = serial.Serial(
            ...
            exclusive=True      # Prevent other apps from interfering
        )
        
        # Explicitly set DTR/RTS low to prevent FPGA reset
        ser.dtr = False
        ser.rts = False
        
        # Critical: Let FTDI chip stabilize
        time.sleep(0.3)
        
        # Optional: Try to reduce FTDI latency timer (Linux/Mac specific)
        try:
            import fcntl
            USBDEVFS_SETLATENCY = 0x5609
            latency = 1  # 1ms instead of default 16ms
            fcntl.ioctl(ser.fileno(), USBDEVFS_SETLATENCY, latency)
            print(f"✅ Set FTDI latency to {latency}ms")
        except:
            pass  # Not critical, just nice to have
        
        # Send initial IDLE byte to prime the connection
        ser.write(b'I')
        ser.flush()
        time.sleep(0.05)
...
def send_event(evt: str):
    """
    Write one byte to UART with proper flushing and timing.
    Updated to ensure complete transmission with FTDI chip.
    """
    b = MAP.get(evt)
    if not b:
        return
    try:
        if ser and ser.is_open:
            ...
            time.sleep(0.025)    # 25ms ensures byte is fully transmitted
    except Exception as e:
...
def draw_hud(frame, y0_pix_raw, yhat, jump_thr, duck_thr, state):
    ...
    # Keep the entire bar on-screen with a small margin
    margin = 10
    half = HUD_SCALE // 2
    ...
    # Threshold lines (clip to frame bounds)
    j_pix = int(y0_pix - jump_thr * HUD_SCALE)
    d_pix = int(y0_pix - duck_thr * HUD_SCALE)  # duck_thr negative
    ...
    # Current yhat dot (clip to frame)
...
def main():
    ...
    if not cap.isOpened():
        raise RuntimeError(f"Could not open camera index {chosen_idx}. Quit other apps using the camera and try again.")
    cap.set(cv2.CAP_PROP_FRAME_WIDTH,  FRAME_WIDTH)
    cap.set(cv2.CAP_PROP_FRAME_HEIGHT, FRAME_HEIGHT)
    ...
    # Open UART (auto-detect). Pass a specific port string if you prefer.
        ...
        while True:
            ...
            if pts:
                ...
                if baseline_locked and scale_S is not None and scale_S > 1e-4:
                    # normalized displacement (+ up, - down)
                    ...
                    fired = None
                    # Priority: DUCK -> JUMP -> IDLE (all strictly thresholded)
                    ...
                    if fired:
                        ...
                        if fired != last_emitted:
                            ...
                            send_event(fired)   # <-- SERIAL SEND HERE
                            last_emitted = fired

                    # Only drift baseline when truly idle
                    if state == "IDLE":
                    ...
                    # Not calibrated: open palm above head to lock baseline
